...
*   Responses return `output`, while Chat Completions return a `choices` array.
...
*   Reasoning is different. Instead of `reasoning_effort` in chat versus reasoning.effort\` in Responses. Read more details in the [reasoning](/docs/guides/reasoning) guide.
*   The Responses SDK has an `output_text` helper, which Chat Completions does not have.
